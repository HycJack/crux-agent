...
// MessageCounter counts tokens in messages, system prompts, and tool definitions.
...
// Includes per-message overhead (role, formatting, separators) ≈ 4 tokens.
func (mc *MessageCounter) CountMessage(msg core.Message) int {
	const messageOverhead = 4

	switch m := msg.(type) {
	case core.UserMessage:
		return messageOverhead + mc.counter.CountTokens("user") + mc.countContent(m.Content)
	case core.AssistantMessage:
		tokens := messageOverhead + mc.counter.CountTokens("assistant")
		for _, block := range m.Content {
			tokens += mc.countContentBlock(block)
		}
		return tokens
	case core.ToolResultMessage:
		tokens := messageOverhead + mc.counter.CountTokens("tool")
		for _, block := range m.Content {
			tokens += mc.countContentBlock(block)
		}
		return tokens
...
// AsTokenCounter returns a function suitable for engine.CompactionConfig.TokenCounter.
// The returned function accepts (systemPrompt, messages, tools) → token count.
func (mc *MessageCounter) AsTokenCounter() func(string, []core.Message, []core.Tool) int {
